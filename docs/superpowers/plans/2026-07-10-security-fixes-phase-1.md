...
> "Before we scrub the API key, please rotate it at TheGamesDB (log in → API keys → revoke `REDACTED-thegamesdb-api-key`, create a new one). Paste the new key here or, better, store it in `/etc/gameTracker/env` (or wherever your prod app reads env vars) as `THEGAMESDB_API_KEY=<newkey>`. Reply 'rotated' with the new key or the env var path when done."
...
echo 'REDACTED-thegamesdb-api-key==>REDACTED' > /tmp/keyscrub.txt
